...
function displayHighlightedDocument() {
  ...
  if (highlightedHtml) {
    ...
    if (documentInfo.filename) {
      ...
      if (titleElement) {
        titleElement.textContent = `${documentInfo.documentType}`;
      }
    }
  } else {
    contentDiv.innerHTML =
      '<p>No document analysis data found. Please <a href="/">upload a document</a> first.</p>';
  }
}
...
function showClausePopup(clauseIndex) {
  ...
  if (clause.suggestions && clause.suggestions.length > 0) {
    ...
    suggestions.innerHTML =
      "<p><strong>This appears to be a standard clause.</strong></p>";
  }
